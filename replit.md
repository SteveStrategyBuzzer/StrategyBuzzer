...
**Plan-Driven Match Flow (#84 — Phase 1 partial):** La banque persistante (#81) a déjà branché le pipeline plan-aware côté `GameServerQuestionPipeline::initMatch()` (Master mode + Node-initiated). Phase 1 de #84 ferme la double-génération évidente côté Duo en supprimant de `LobbyService::startGame()` (branche Duo, lignes ~800-919) : (a) la pré-génération synchrone de Q1 via `QuestionService::generateQuestion()`, (b) le push de Q1 au Game Server via `gameServerService->sendQuestions()`, (c) l'appel au 3e planificateur fantôme `QuestionPlanBuilder::build()`, (d) le dispatch de `GenerateMultiplayerQuestionsJob`, (e) l'init de l'`AntiDuplicationCacheService` (consommé uniquement par ce job). Conséquence : quand `gameServerService->startGame()` déclenche `GameOrchestrator.startGame()` côté Node, `room.state.questions.length === 0` et Node retombe sur `initQuestionPipeline()` → `POST /api/game-server/init` → `GameServerQuestionPipeline::initMatch()` qui construit UN plan unique persisté en base, sert Q1 depuis le plan, et dispatche `GenerateGameServerQuestionsJob` pour la suite. Source unique = pipeline. **Cartographie League (Phase 1)** : la branche League dans `LobbyService::startGame()` (lignes 956-960) ne fait QUE `status='starting'` ; elle ne déclenche jamais `gameServerService->startGame()`. Pour `league_individual`, les matchs sont créés directement avec `status='playing'` par `LeagueIndividualService::createMatch()` qui appelle `gameStateService->initializeGame(...)`, et les questions sont stockées dans la colonne JSONB `game_state['questions']` puis lues par `LeagueIndividualController::fetchQuestionJson()` — sans aucune ré-génération à runtime. Pour `league_team`, `LeagueTeamController::startMatch()` (ligne 1130) crée le match + alloue une room Node via `createRoom('LEAGUE_TEAM')` (ligne 1274) uniquement pour live-stats / WS (commentaire ligne 1268 : "The legacy REST gameplay loop continues to drive question/buzz/answer state for now") ; les questions sont servies par `LeagueTeamController::getQuestion()` → `leagueTeamService->getNextQuestion()`. Conclusion : League utilise une boucle REST entièrement séparée du pipeline. **Backlog Phase 3** : `GenerateMultiplayerQuestionsJob` + `QuestionPlanBuilder` deviennent code mort à supprimer après #86. Les chemins League REST (`LeagueIndividualService::createMatch` via `gameStateService->initializeGame` + `LeagueTeamService::getNextQuestion`) doivent aussi être branchés sur le plan.
