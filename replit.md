...
- **Best of 3 Match System Implementation**: Complete overhaul of gameplay mechanics. Changed from 5-round system to "best of 3 matches" where one match = all selected questions (e.g., 30 questions = 1 match). Players must win 2 matches out of 3 to win the game. Added round_result.blade.php transition page showing match score (X-0, 1-1, etc.). Upon winning, player advances to next level with stronger opponents, progressing toward boss battles at level 10, 20, etc., and the ultimate "Cerveau" boss at level 100.
- **Global Statistics System**: Implemented comprehensive statistics tracking across all matches. Added global_stats session variable to accumulate question results (correct/incorrect/unanswered) across all rounds. Game result page now displays: theme, level, round score (X-X), total correct/incorrect/unanswered counts with percentages, and global efficiency metric.
- **Round Result Page Enhancement**: Modified round_result.blade.php to display Score (player-opponent), Lives remaining, Round Efficiency percentage, and Opponent Level. Controller now calculates and saves round statistics before resetting for next round.
