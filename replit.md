...
**Dev-Only Test-Support Endpoints + E2E Regression Suite (Task #41):** Two routes exist solely to unblock automated E2E coverage of the multiplayer happy path (Firebase OAuth-only login otherwise blocks scripted clients). Both are gated by `app()->environment('production')` at registration time AND re-checked inside the controller, and both are CSRF-exempt (`__test/*` in `VerifyCsrfToken::$except`). `POST /__test/login` creates (if missing) and logs in the fixture user `E2E-0001` / `e2e-fixture@strategybuzzer.local`. `POST /__test/duo/setup-bot-match` (auth-required) cancels stale matches, creates a fresh Duo match vs the seeded bot user (`BT-0001`), reuses the single Game Server room that `LobbyService::createLobby()` already allocates (no duplicate room creation), and seeds `session('game_state')` so the player lands directly on `/game/duo/intro` ready to socket-connect/join_room. Both endpoints are implemented in `App\Http\Controllers\TestSupportController`.

Two persisted, CI-runnable regression tests guard the join_room contract:
1. `npm run test:e2e:duo-join-browser` → `tests/e2e/duo-join-room.browser.spec.js` — true browser-driven test using Playwright + system Chromium (`/nix/store/...-chromium-.../bin/chromium`, installed via the package-management skill). It logs in, sets up the bot match, then walks real Chromium through the full user journey: `/duo/lobby/<code>` → `/game/duo/intro` → `/game/duo/question`. It captures every browser console message AND `pageerror` across all three pages, and asserts (a) at least one `[GameplayRuntime] Joined room: <uuid>` message AND (b) zero messages matching `VALIDATION_ERROR` / `Invalid join_room` / `Cannot join_room`. This exercises the exact `public/js/GameplayRuntime.js` payload-shaping surface that the original regression broke.
2. `npm run test:e2e:duo-join` → `tests/e2e/duo-join-room.test.js` — supplemental Node `socket.io-client` contract test that does the same setup, opens a raw socket with the issued JWT, emits a hand-crafted `join_room`, and asserts the same forbidden patterns are absent in server `error` / `error_message` / `room_error` events. Useful for fast contract-level regression detection without launching a browser.
