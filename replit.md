...
**Persistent Question Bank (Postgres):** Canonical multilingual question store under `App\Services\QuestionBank\*` with 3 tables (`question_groups`, `question_translations`, `match_question_plans`). The bank is the nominal path for every match. `MatchQuestionPlanner::buildMatchPlan()` produces an ordered match plan using deterministic largest-remainder allocation, enforcing cognitive-mix and sub-domain quotas. Plans/picks identify a canonical `question_group_id` for conceptual question consistency across languages. Anti-clone and anti-overuse guards are enforced.

**Embedded Seed Pool:** Last-resort static fallback consumed by `App\Services\SeedQuestionPoolService` when bank and cache are dry. It consists of 32 concepts × 10 languages = 320 questions, each tagged with `concept_id`, `sub_domain`, `cognitive_type`, and `niveau_band`. The service supports filtering with graceful widening.
