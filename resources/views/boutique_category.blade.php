...
.pill b { color: #fff; }
.pill img { width: 28px; height: 28px; min-width: 28px; min-height: 28px; object-fit: cover; object-position: center; clip-path: circle(50%); flex-shrink: 0; }
            ...
            @foreach($intelligencePacks ?? [] as $pack)
                <div class="card coin-pack {{ ($pack['popular'] ?? false) ? 'popular' : '' }}">
                    ...
                    <div class="coin-icon-display" style="margin-top:-20px;margin-bottom:-40px;">
                        <img src="{{ asset('images/coin-intelligence.png') }}" alt="" style="width:128px;height:128px;object-fit:contain;">
                    </div>
            ...
            @foreach($competencePacks ?? [] as $pack)
                <div class="card coin-pack {{ ($pack['popular'] ?? false) ? 'popular' : '' }}">
                    @if($pack['popular'] ?? false)
                        <div class="popular-badge">⭐ {{ __('Populaire') }}</div>
                    @endif
                    <div class="coin-icon-display" style="margin-top:-12px;margin-bottom:-24px;">
                        <img src="{{ asset('images/skill_coin.png') }}" alt="" style="width:100px;height:100px;object-fit:contain;">
                    </div>
                    <div class="coin-amount">{{ number_format($pack['coins'] ?? 0) }}</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Packs de coins (affichage)
    document.querySelectorAll('.coin-price').forEach(el => {
        const priceText = (el.textContent || '').trim();
        const match = priceText.match(/[\d.,]+/);
        if (match) {
            el.textContent = currencySymbol + match[0];
        }
    });

    // Boutons des modes de jeu (si un prix $xx.xx est affiché)
    document.querySelectorAll('.master-card button').forEach(btn => {
