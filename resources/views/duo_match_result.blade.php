...
$decidedBy    = $match_result['decided_by'] ?? 'rounds';
  $questsUnlocked = $quests_unlocked ?? [];
...
<div class="result-card">
    ...
    <div class="stats-grid">
        @php
            $me = auth()->id();
            $stats = $player_stats_summary[$me] ?? null;
        @endphp

        <div class="stat-box">
            <div class="stat-icon">⚡</div>
            <div class="stat-value">{{ $stats["accuracyPct"] ?? 0 }}%</div>
            <div class="stat-label">Précision</div>
        </div>

        <div class="stat-box">
            <div class="stat-icon">🎯</div>
            <div class="stat-value">{{ $stats["buzzWinCount"] ?? 0 }}</div>
            <div class="stat-label">Buzz gagnés</div>
        </div>

        <div class="stat-box">
            <div class="stat-icon">⏱</div>
            <div class="stat-value">{{ isset($stats["avgBuzzMs"]) ? $stats["avgBuzzMs"] : 0 }} ms</div>
            <div class="stat-label">Temps moyen buzz</div>
        </div>

        <div class="stat-box">
            <div class="stat-icon">📊</div>
            <div class="stat-value">{{ $stats["correctAnswers"] ?? 0 }}/{{ $stats["answersSubmitted"] ?? 0 }}</div>
    ...
    <div class="rewards-box">
      ...
      @endif
    @if(count($questsUnlocked) > 0)
    <div class="rewards-box" style="margin-top:18px;">
        <div class="rewards-title">🏅 {{ __("Quêtes débloquées") }}</div>
        <div class="rewards-row">
            @foreach($questsUnlocked as $quest)
            <div class="reward-item">
                <div class="reward-icon">{{ $quest->badge_emoji ?? "🏅" }}</div>
                <div class="reward-val" style="font-size:1rem;">{{ $quest->name }}</div>
                <div class="reward-lbl">+{{ $quest->reward_coins ?? 0 }} {{ __("pièces") }}</div>
            </div>
            @endforeach
        </div>
    </div>
