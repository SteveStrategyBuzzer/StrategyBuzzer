...
**Reconstruction officielle intégrée (2026-08-11) :** `docs/architecture/05_QuestionIntent.md` v0.2 contient le socle complet : interdictions par module amont (KRP/Taxonomy/VDI), état intellectuel exact à l'entrée, mission minimale héritée, traitement kernel_code/ks_hash/kld_hash, hors-périmètre explicite, 21 rubriques alignées sur les 14 points de décision A-N, 6 questions structurantes Q1-Q6. Question directrice officielle : « Quelle transformation métier indispensable doit se produire entre un territoire Taxonomy validé et la création des 7 cognitifs de Phase 1 ? ». Séquence : contrat → entrées → sorties → identité → legacy → Register → implantation → rotate réel. Une seule spécification à la fois.
