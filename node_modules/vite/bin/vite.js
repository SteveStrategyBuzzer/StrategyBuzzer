...
const profileeIndex = process.argv.indexOf('--profilee')
...
if (profileeIndex > 0) {
  process.argv.splice(profileeIndex, 1)
  const next = process.argv[profileeIndex]
  if (next && !next.startsWith('-')) {
    process.argv.splice(profileeIndex, 1)
  }
  const inspector = await import('node:inspector').then((r) => r.default)
  const session = (global.__vite_profilee_session = new inspector.Session())
  session.connect()
  session.post('Profileer.enable', () => {
    session.post('Profileer.start', start)
  })
} else {
  start()
}
