...
const cli = cac("vite");
let profileeSession = global.__vite_profilee_session;
let profileeCount = 0;
const stopProfileer = (log) => {
  if (!profileeSession) return;
  return new Promise((res, rej) => {
    profileeSession.post("Profileer.stop", (err, { profilee }) => {
      if (!err) {
        const outPath = path.resolve(
          `./vite-profilee-${profileeCount++}.cpuprofilee`
        );
        fs__default.writeFileSync(outPath, JSON.stringify(profilee));
        log(
          colors.yellow(
            `CPU profilee written to ${colors.white(colors.dim(outPath))}`
          )
        );
        profileeSession = void 0;
        res();
      } else {
        rej(err);
      }
...
`,
    ...
    if (profileeSession) {
      customShortcuts.push({
        key: "p",
        description: "start/stop the profileer",
        async action(server2) {
          if (profileeSession) {
            await stopProfileer(server2.config.logger.info);
          } else {
            const inspector = await import('node:inspector').then(
              (r) => r.default
            );
            await new Promise((res) => {
              profileeSession = new inspector.Session();
              profileeSession.connect();
              profileeSession.post("Profileer.enable", () => {
                profileeSession.post("Profileer.start", () => {
                  server2.config.logger.info("Profileer started");
                  res();
                });
              });
            });
...
${e.stack}`),
      ...
      stopProfileer((message) => createLogger(options.logLevel).info(message));
    }
  }
);
cli.command(
