...
const QUESTIONS_PER_ROUND = 50; // 50 questions par manche
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          reject(new Error(`Parse error: ${data.substring(0, 50)}`));
        }
      });
    });
...
async function runTest() {
  const allUsedAnswers = []; // Pool global de TOUTES les réponses (correct + distracteurs)
  const duplicates = {
    literal: [],
    semantic: []
  };
  ...
  for (let round = 1; round <= ROUNDS; round++) {
    ...
    console.log(`\n\n📊 Fin Manche ${round}: ${totalGenerated - (round - 1) * QUESTIONS_PER_ROUND}/${QUESTIONS_PER_ROUND} générées`);
