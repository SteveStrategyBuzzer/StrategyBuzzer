...
function getOpenCollectiveSponsors(): string
{
    ...
    return implode('', array_map(static function (array $member) use ($customSponsorImages): string {
        $href = htmlspecialchars($member['website'] ?? $member['profilee']);
        $src = $customSponsorImages[$member['MemberId'] ?? ''] ?? $member['image'] ?? (strtr($member['profilee'], ['https://opencollective.com/' => 'https://images.opencollective.com/']).'/avatar/256.png');
