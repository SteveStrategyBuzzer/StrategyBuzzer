...
class LinkedInProvider extends AbstractProvider implements ProviderInterface
{
     ...
     * @var array
     */
    protected $scopes = ['r_liteprofilee', 'r_emailaddress'];
    ...
    {
        $basicProfilee = $this->getBasicProfilee($token);
        $emailAddress = $this->getEmailAddress($token);

        return array_merge($basicProfilee, $emailAddress);
    }

    /**
     * Get the basic profilee fields for the user.
     *
     * @param  string  $token
     * @return array
     */
    protected function getBasicProfilee($token)
    {
        $fields = ['id', 'firstName', 'lastName', 'profileePicture(displayImage~:playableStreams)'];

        if (in_array('r_liteprofilee', $this->getScopes())) {
    ...
    protected function mapUserToObject(array $user)
    {
        ...
        $images = (array) Arr::get($user, 'profileePicture.displayImage~.elements', []);
