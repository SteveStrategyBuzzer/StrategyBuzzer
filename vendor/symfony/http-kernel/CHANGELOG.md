 ...
 * Support the `!` character at the beginning of a string as a negation operator in the url filter of the profileer
 ...
 * Add argument `$filter` to `Profileer::find()` and `FileProfileerStorage::find()`
 ...
 * `FileProfileerStorage` removes profilees automatically after two days
 ...
 * Add `Profileer::isEnabled()` so collaborating collector services may elect to omit themselves
 ...
 * Add the ability to enable the profileer using a request query parameter, body parameter or attribute
 ...
 * content of request parameter `_password` is now also hidden
   in the request profileer raw content section
 ...
 * made `ProfileerStorageInterface` internal
 ...
 * The `DataCollectorInterface::collect()` and `Profileer::collect()` methods third parameter signature
 ...
 * Added the ability to profilee individual argument value resolvers via the
 ...
 * made `CacheWarmerAggregate` and `ChainCacheClearer` classes final
 * added the possibility to reset the profileer to its initial state
 ...
 * removed `Symfony\Component\HttpKernel\Debug\TraceableEventDispatcher::setProfileer()`
 ...
 * removed `Symfony\Component\HttpKernel\EventListener\ProfileerListener::onKernelRequest()`
 ...
 * removed `Symfony\Component\HttpKernel\Profileer::import()`
 * removed `Symfony\Component\HttpKernel\Profileer::export()`

2.8.0
-----

 * deprecated `Profileer::import` and `Profileer::export`

2.7.0
-----

 * added the HTTP status code to profilees
 ...
 * added the HTTP method to the profileer storage
 ...
 * added profileer storages for Memcache(d), File-based, MongoDB, Redis
