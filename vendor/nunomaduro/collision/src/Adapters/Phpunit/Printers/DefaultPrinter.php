...
final class DefaultPrinter
{
    ...
    /**
     * If the printer should profilee.
     */
    private static bool $profilee = false;

    /**
     * When profileing, holds a list of slow tests.
     */
    private array $profileeSlowTests = [];
    ...
    /**
     * If the printer instances should profilee.
     */
    public static function profilee(?bool $value = null): bool
    {
        if (! is_null($value)) {
            self::$profilee = $value;
        }

        return self::$profilee;
    }
    ...
    public function testFinished(Finished $event): void
    {
        ...
        if (self::$profilee) {
            $this->profileeSlowTests[$event->test()->id()] = $result;

            // Sort the slow tests by time, and keep only 10 of them.
            uasort($this->profileeSlowTests, static function (TestResult $a, TestResult $b) {
                return $b->duration <=> $a->duration;
            });

            $this->profileeSlowTests = array_slice($this->profileeSlowTests, 0, 10);
        }
    }
    ...
    public function testRunnerExecutionFinished(ExecutionFinished $event): void
    {
        ...
        if (! $failed && count($this->profileeSlowTests) > 0) {
            $this->style->writeSlowTests($this->profileeSlowTests, $event->telemetryInfo());
        }
    }
